...
class Fetch {
  ...
  // Post method
  static async post(url, body, arg = { authHeader: false }) {
  ...
  // Put method
  static async put(url, body, arg = { authHeader: false }) {
  ...
  // Delete method
  static async delete(url, arg = { authHeader: false }) {
    delete this.options.body;
...
//   // Open file
//   static async open(url, arg = { type: "", authHeader: false }) {
//     this.options.method = "GET";
...
//   }
  ...
  // Parse JSON reponse
  static async parseJsonReponse(response) {
  ...
  // Parse Blob
  static async parseBlob(response) {
    if (!response.ok || response.status == 401) {
      let currentRouteName = _get(Router, "history.current.name");
      if (currentRouteName != "login") {
        Router.replace({ name: "login" });
